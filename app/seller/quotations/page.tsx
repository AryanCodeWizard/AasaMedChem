...
interface Quotation {
  ...
  total_inr: string; created_at: string; notes: string | null;
}
    ...
    <>
      <div className="dashboard-header">
        <div>
          ...
          <p className="page-subtitle">{total} quotation{total !== 1 ? 's' : ''}</p>
        </div>
        <Link href="/seller/products" className="btn btn-primary">+ New Quotation</Link>
      </div>
      ...
      <div className="dashboard-content">
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <div className="empty-subtitle">{status !== 'all' ? `No ${status} quotations` : 'Place your first quotation from the products page'}</div>
          ...
          <>
            <div className="table-wrapper">
              <table>
                <thead><tr><th>Quotation #</th><th>Total</th><th>Status</th><th>Notes</th><th>Date</th><th>Action</th></tr></thead>
                <tbody>
                  {quotations.map(q => (
                    <tr key={q.id}>
                      ...
                      <td className="text-sm text-muted" style={{ maxWidth: 200 }}><div className="truncate">{q.notes || '—'}</div></td>
